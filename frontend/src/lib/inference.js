 ...
 */
export function buildFeatureMatrix(candles, lookback = 60) {
    ...
    function rsi(closes, period = 14) {
        ...
        const result = [50]; // first value is neutral
    ...
    for (let i = warmup; i < slice.length; i++) {
        ...
        matrix.push([
            ...
            slice[i].open,          // 3  open
            volumes[i],             // 4  volume
            rsiIdx >= 0 ? (rsiVals[rsiIdx] ?? 50) : 50,   // 5  rsi
            ...
            50,                     // 14 sentiment_score (neutral default; replace with live value)
        ]);
    }
    return matrix;
}
